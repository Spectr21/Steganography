...
def open_file(flag, tx, arr):
    if tx == '':
        showinfo("ERROR", 'No text found')
        return 0
    global root, frame_mid
    fn = ''
    fn = fd.askopenfilename()
    if fn == '':
        showinfo("ERROR", "File not chosen")
        return 0
    for i in arr:
        i.pack_forget()
...
def encode():
    ...
    ent = Entry()
    ent.pack()
    btn = Button(root, text='Choose file', font='50', command=lambda: open_file(1, ent.get(), [tx, ent, btn]),
                 height=20, width=20)
    btn.pack()
...
def decode():
    ...
    ent1 = Entry()
    ent1.pack()
    btn = Button(root, text='Choose file', font='50', command=lambda: open_file(0, ent1.get(), [tx1, ent1, btn]),
                 height=20,
                 width=20)
    btn.pack()
    frame_back = Frame(root, bg='white', bd=5)
