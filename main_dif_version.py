...
def new_window(*p):
    global root, btn, frame_top, frame_bottom, frame_new
    for i in p:
        i.destroy()
    ...
    tx = Label(text='чтооооооож')
    tx.pack()
    tx.destroy()
    # root.after(10000, encode())
    # btn = Button(root, text='Choose file', command=lambda: open_file(ent.get()), height=20, width=20)
    btn10 = Button(root, text='new', command=lambda: new_back(tx, btn10), height=20, width=20)
    btn10.pack()
    # btn.pack()
...
def open_file(tx):
    ...
    # ar = multiprocessing.Process(target=enc)
    # ar.start()
    # ar.join()
    lbl.pack()
    btn.pack()
...
def encode():
    ...
    btn1 = Button(frame_back, text="Back", command=lambda: back(btn, btn1, frame_back, tx, ent), height=2, width=10)
    btn1.pack()
    btn10 = Button(frame_new, text='new', command=lambda: new_window(btn, btn1, tx, ent, btn10), height=20,
                   width=20)
    btn10.pack()
    root.mainloop()
...
frame_new = Frame(root, bg='#ffb700', bd=5)
frame_new.place(relx=0.8, rely=0.1, relwidth=0.1, relheight=0.1)
